...
impl Tree {
    ...
    pub fn train(&mut self, dataset: &EncryptedDataset, public_key: &PublicKey, ctx: &Context, timing: &TimingCollector) {
        for sample in dataset.records.iter() {
            let ctree = time_operation!(timing, "Compile tree", {
                CTree::new(self, &sample.features, public_key, ctx)
            });

            let selector = time_operation!(timing, "Evaluate tree", {
                ctree.evaluate(public_key, ctx)
            });

            if DEBUG {
                let private_key = key(ctx.parameters());
                time_operation!(timing, "Print tree", {
                    ctree.print(&private_key, ctx);
                });
                println!("Selector: {}", private_key.decrypt_lwe(&selector, ctx));
            }

            time_operation!(timing, "Leaves update", {
    ...
    pub fn train_fully_oblivious(
        ...
        time_operation!(timing, "Leaves majority", {
            self.leaves_majority(public_key, ctx);
        });

        let class = time_operation!(timing, "Blind read", {
            let final_leaves_lut = LUT::from_vec_of_lwe(&self.final_leaves, public_key, ctx);
            public_key.blind_array_access(&selector, &final_leaves_lut, ctx)
        });
