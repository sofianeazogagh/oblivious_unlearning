...
fn example_private_training() {
    ...
    for _ in 0..NUM_EXPERIMENTS {
        ...
        let decrypted_counts = forest.iter().map(|(tree, luts_samples)| {
            decrypt_counts(luts_samples.clone(), N_CLASSES, TREE_DEPTH, &private_key, &ctx)
        }).collect::<Vec<_>>();
        ...
        for i in 0..M{
            ...
            for j in 0..2u32.pow(TREE_DEPTH as u32){
                ...
                for k in 1..N_CLASSES{
                    if decrypted_counts[i as usize][j as usize][k as usize] > decrypted_counts[i as usize][j as usize][label as usize]{
                        label = k;
                    }
                }
                labels.push(label);
        ...
        if EXPORT_FOREST {
            for i in 0..M {
                forest[i as usize].0.save_to_file(&format!("iris_forest/tree_{}.json", i), &ctx);
            }
        }
        ...
        for sample_idx in  0..test_size{
            ...
            let true_label = private_key.decrypt_lwe(&test_dataset.records[sample_idx as usize].class, &ctx);
...
pub fn decrypt_counts(lut_samples: Vec<Vec<LUT>>, n_classes:u64, tree_depth:u64, private_key: &PrivateKey, ctx: &Context) -> Vec<Vec<u64>> {
