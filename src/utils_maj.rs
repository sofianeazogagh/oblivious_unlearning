...
pub(crate) trait Majority {
    fn blind_count_extra(&self, lwes: &[LWE], ctx: &Context, n_classes: u64) -> Vec<ByteLWE>;
    fn blind_majority_extra(&self, lwes: &[LWE], ctx: &Context, n_classes: u64, timing: &TimingCollector) -> LWE;
...
impl Majority for PublicKey {
    ...
    fn blind_majority_extra(&self, lwes: &[LWE], ctx: &Context, n_classes: u64, timing: &TimingCollector) -> LWE {
        let count = time_operation!(timing, "Blind count", {
            self.blind_count_extra(lwes, ctx, n_classes)
        });

        let maj = time_operation!(timing, "Blind argmax", {
            self.blind_argmax_blwe_lwe(&count, ctx)
        });
        maj
    }
